    ...
    Protected Overrides Sub Poller()
        ...
        Try
                        ...
                        For Each fi In di.GetFiles(.Mask, so)
                            ...
                            If fi.LastWriteTime > earliestLastModified Then
                                ...
                                If fi.LastWriteTime < latestLastWrittenTo Then
                                    ...
                                    fti.PulledTrigger = fi
                                    .TriggersFound.Add(fi.FullName, fti)
                            ...
                            For Each fi In subdi.GetFiles(.Mask, so)
                                ...
                                If fi.LastWriteTime > earliestLastModified Then
                                    ...
                                    If fi.LastWriteTime < latestLastWrittenTo Then
                                        ...
                                        Dim fti As New FoundTriggerInfo
                                        fti.PulledTrigger = fi
                                        aWatcherInfo.TriggersFound.Add(fi.FullName, fti)
            ...
            If FoundValidTrigger = True Then
                ...
                '--- see if there is already Crumbler info in the file's ADS
                For Each de As DictionaryEntry In _triggersFound
                    ...
                    Dim fiTrigger As FileInfo = CType(fti.PulledTrigger, FileInfo)
                    ...
                    Try
                        ...
                        '--- Trail Description
                        sTrailDescription = GetADSInfo(fiTrigger.FullName, "BC_TrailDescription")

                        '--- Trail Label
                        sTrailLabel = GetADSInfo(fiTrigger.FullName, "BC_TrailLabel")

                        '--- Trail UID
                        sTrailUid = GetADSInfo(fiTrigger.FullName, "BC_UID")
                        
                        '--- create the Crumbler
                        fti.Crumbler = New Crumbler(sTrailLabel, sTrailDescription, sTrailUid)

                    Catch ex As Exception

                        fti.Crumbler = Nothing
                ...
                '--- send the messages to the ATJ queue
                CreateMessages()
