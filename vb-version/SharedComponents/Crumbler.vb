...
Public Class Crumbler
    ...
    ''' <summary>
    ''' Returns a basic Trail Info node, optionally containing soem text
    ''' </summary>
    ''' <param name="TextContents"></param>
    ''' <returns></returns>
    ''' <remarks>SDCP 18-Mar-2008</remarks>
    Public Function GetBasicTrailInfo(Optional ByVal TextContents As String = "") As XmlNode

        Dim xDoc As XmlDocument = New XmlDocument
        xDoc.LoadXml("<info/>")

        If TextContents <> "" Then
            xDoc.DocumentElement.InnerText = TextContents
        End If

        _Info = xDoc.DocumentElement
        Return _Info

    End Function

    ''' <summary>
    ''' Append to the info node
    ''' </summary>
    ''' <param name="MessageLabel"></param>
    ''' <param name="MessageValue"></param>
    ''' <remarks>ML 24-April-2008</remarks>
    Public Sub AddInfo(ByVal MessageLabel As String, ByVal MessageValue As String)

        If _Info Is Nothing Then
            _Info = GetBasicTrailInfo()
        End If

        Dim xChildNode As Xml.XmlNode = _Info.OwnerDocument.CreateNode(XmlNodeType.Element, MessageLabel, "")
        xChildNode.InnerText = MessageValue
        _Info.AppendChild(xChildNode)
